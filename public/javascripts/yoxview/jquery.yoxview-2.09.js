    ...
    $.fn.extend({
        yoxview: function(opt) 
        {
            ...
            var defaults = {
                ...
                parentElement: $(top.document.getElementsByTagName("body")[0]), // The element which holds the popup
...
{  
    ...
    var yoxviewApi = this,
        $ = jQuery, // Ensure the dollar sign stands for jQuery, in case other JS libraries are loaded, that use it.
        ajaxLoader,
        cacheVars = {},
        cacheImg = new Image(),
        countDisplay,
    ...
    {
        var view = $(view);
        view.data("yoxview", {viewIndex : viewIndex, cacheVars: {cachedImagesCount: 0, cacheDirectionForward: true, cacheBufferLastIndex: null, currentCacheImg: 0 }});
...
//                on the keyboard, specify the Next button. If no button should be display, leave blank.
    ...
    this.zoom = function(e){
        var currentItem = images[currentItemIndex].media;
        if (!currentItem.contentType || currentItem.contentType === 'image')
        {
            var panelData = newPanel.data("yoxviewPanel");
            var panelSize = { width: newPanel.width(), height: newPanel.height() };
            
            if (!panelData.isZoomed)
            {
                panelData.image.css("position", "absolute");
                panelData.image.animate({
                    width: currentItem.width,
                    height: currentItem.height,
                    left: Math.round((panelSize.width - currentItem.width) / 2),
                    top: Math.round((panelSize.height - currentItem.height) / 2)
                }, "fast");
                panelData.isZoomed = true;
                newPanel.bind("mousemove", zoomPan);
            }
            else
            {
                panelData.image.animate({
                ...
                panelData.isZoomed = false;
                newPanel.unbind("mousemove");
            }
        }
    };
    this.play = function()
